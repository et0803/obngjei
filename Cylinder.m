%https://ww2.mathworks.cn/matlabcentral/fileexchange/13995-cylinder?s_tid=prof_contriblnk

function [Cylinder EndPlate1 EndPlate2] = Cylinder(X1,X2,r,n,cyl_color,endPlaneColor,closed,lines,faceAlpha,endPlaneFaceAlpha)
 %画图且返回句柄时，就是在当前figure上画的。
...
% from Origin
Cylinder=mesh(xx1,xx2,xx3,'FaceAlpha',faceAlpha);
...
if closed==1
    set([EndPlate1 EndPlate2],'FaceColor',endPlaneColor,'FaceAlpha',endPlaneFaceAlpha);
else
    EndPlate1=[];
    EndPlate2=[];
end
...
% demo 返回值都是已经画好的图的handle, 只能用来设置没有关闭的figure的图形句柄的属性。
% X1=[10 10 10];
% X2=[35 20 40];
% r=1;
% n=20;
...
% lines=0;
% faceAlpha=0.6;
% Cylinder(X1+20,X2,r,n,cyl_color,endPlaneColor,closed,lines,faceAlpha);
% X1=[0 0 0];
% X2=[35 20 40];
% Cylinder(X1+20,X2,r,n,cyl_color,endPlaneColor,closed,lines,faceAlpha);